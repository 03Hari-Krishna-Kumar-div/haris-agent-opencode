...
var warnedOnce = false, cachedSid;
...
function normalizeHostname(hostname2) {
  let h = hostname2.startsWith("[") && hostname2.endsWith("]") ? hostname2.slice(1, -1) : hostname2;
  if (h.endsWith("."))
    h = h.slice(0, -1);
  return h;
}
function isMetadataIp(hostname2) {
  try {
    const probe = new URL(`http://${hostname2}`);
...
async function resolvesToBlockedIp(hostname2) {
  try {
    const dns = await import("node:dns");
    const { resolve4, resolve6 } = dns.promises;
    const v4Check = resolve4(hostname2).then((addresses) => addresses.some((addr) => BLOCKED_METADATA_HOSTS.has(addr)), () => false);
    const v6Check = resolve6(hostname2).then((addresses) => addresses.some((addr) => {
...
async function validateNavigationUrl(url) {
  ...
  const hostname2 = normalizeHostname(parsed.hostname.toLowerCase());
  if (BLOCKED_METADATA_HOSTS.has(hostname2) || isMetadataIp(hostname2) || isBlockedIpv6(hostname2)) {
    throw new Error(`Blocked: ${parsed.hostname} is a cloud metadata endpoint. Access is denied for security.`);
  }
  const isLoopback = hostname2 === "localhost" || hostname2 === "127.0.0.1" || hostname2 === "::1";
  const isPrivateNet = /^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.)/.test(hostname2);
  if (!isLoopback && !isPrivateNet && await resolvesToBlockedIp(hostname2)) {
...
function runBoundedChromiumReinstall(installRoot, timeoutMs = XPROTECT_REINSTALL_TIMEOUT_MS) {
  return new Promise((resolve5) => {
    ...
    try {
      ...
      resolve5({ ok: false, reason: `spawn-error: ${err instanceof Error ? err.message : String(err)}` });
      return;
    }
    let stderrTail = "";
    ...
    child.on("error", (err) => {
      ...
      settled = true;
      clearTimeout(timer);
      resolve5({ ok: false, reason: `spawn-error: ${err.message}` });
    });
    child.on("exit", (code) => {
      if (settled)
        return;
      ...
      clearTimeout(timer);
      if (code === 0) {
        resolve5({ ok: true, exitCode: code });
      } else {
        resolve5({
          ok: false,
...
var __dirname = "C:\\Users\\ADMIN\\gstack\\browse\\src", NEGATIVE_SIGNATURES, POSITIVE_SIGNATURES, REVISION_DIR_RE, XPROTECT_REINSTALL_TIMEOUT_MS = 120000, healAttempted = false;
...
function logTelemetry(payload) {
  ...
  ensureDir().then(() => fs6.appendFile(telemetryFile(), JSON.stringify(enriched) + `
`, "utf8")).catch(() => {});
}
...
function probePoisonedChromiumBundle(chromiumExecutablePath) {
  ...
  const customPath = process.env.GSTACK_CHROMIUM_PATH;
  if (customPath && path7.resolve(chromiumExecutablePath) === path7.resolve(customPath)) {
    return;
  }
  const chromeContentsDir = path7.resolve(path7.dirname(chromiumExecutablePath), "..");
  const chromePlist = path7.join(chromeContentsDir, "Info.plist");
  if (!fs7.existsSync(chromePlist))
    return;
  if (!fs7.readFileSync(chromePlist, "utf-8").includes("GStack Browser"))
    return;
  const appDir = path7.resolve(chromeContentsDir, "..");
  const revisionDir = path7.resolve(appDir, "..", "..");
  if (/^chromium-\d+$/.test(path7.basename(revisionDir))) {
    fs7.rmSync(revisionDir, { recursive: true, force: true });
  } else {
    fs7.rmSync(appDir, { recursive: true, force: true });
    for (const marker of ["INSTALLATION_COMPLETE", "DEPENDENCIES_VALIDATED"]) {
      fs7.rmSync(path7.join(path7.dirname(appDir), marker), { force: true });
    }
  }
...
async function resolveDisconnectCause(browser) {
  ...
  if (proc && proc.exitCode === null && proc.signalCode === null) {
    await new Promise((resolve5) => {
      const timer = setTimeout(resolve5, 1000);
      proc.once("exit", () => {
        clearTimeout(timer);
        resolve5();
      });
...
class BrowserManager {
  ...
  findExtensionPath() {
    ...
    const candidates = [
      ...
      path7.resolve(__dirname, "..", "..", "extension"),
      path7.join(process.env.HOME || "", ".claude", "skills", "gstack", "extension"),
      (() => {
        const stateFile = process.env.BROWSE_STATE_FILE || "";
        if (stateFile) {
          const repoRoot = path7.resolve(path7.dirname(stateFile), "..");
          return path7.join(repoRoot, ".claude", "skills", "gstack", "extension");
        }
        return "";
      })()
    ].filter(Boolean);
    for (const candidate of candidates) {
      try {
        if (fs7.existsSync(path7.join(candidate, "manifest.json"))) {
          return candidate;
        }
      } catch (err) {
  ...
  async launchHeaded(authToken) {
    ...
    if (extensionPath) {
      ...
      if (authToken) {
        ...
        const gstackDir = path8.join(process.env.HOME || "/tmp", ".gstack");
        mkdirSecure(gstackDir);
        const authFile = path8.join(gstackDir, ".auth.json");
        try {
    ...
    fs7.mkdirSync(userDataDir, { recursive: true });
  ...
  closeRaceMs = 5000;
  async close() {
    const raceTimeout = (ms) => new Promise((resolve5) => {
      const t = setTimeout(() => resolve5(false), ms);
      t.unref?.();
    });
...
var __dirname = "C:\\Users\\ADMIN\\gstack\\browse\\src", PoisonedBundleError;
...
function writeEvalResult(outPath, str, opts) {
  ...
  fs8.mkdirSync(path7.dirname(path7.resolve(outPath)), { recursive: true });
  if (!opts.raw && str.startsWith("data:")) {
    ...
    if (comma !== -1) {
      ...
      if (tokens.includes("base64")) {
        ...
        fs8.writeFileSync(outPath, buf2);
        return buf2.length;
      }
    }
  }
...
function assertJsOriginAllowed(bm, pageUrl) {
  ...
  let hostname2;
  try {
    hostname2 = new URL(pageUrl).hostname;
  } catch {
    return;
  }
  ...
  const allowed = [...importedDomains].some((domain) => {
    ...
    return hostname2 === domain.replace(/^\./, "") || hostname2.endsWith(normalized);
  });
  if (!allowed) {
    throw new Error(`JS execution blocked: current page (${hostname2}) does not match any cookie-imported domain. ` + `Imported cookies for: ${[...importedDomains].join(", ")}. ` + `This prevents cross-origin cookie exfiltration. Navigate to an imported domain or run without imported cookies.`);
  }
}
...
function findInstalledBrowsers() {
  return BROWSER_REGISTRY.filter((browser) => {
    ...
    for (const platform of getSearchPlatforms()) {
      ...
      try {
        const entries = fs9.readdirSync(browserDir, { withFileTypes: true });
        if (entries.some((e) => {
          if (!e.isDirectory() || !e.name.startsWith("Profile "))
            return false;
          const profileDir = path8.join(browserDir, e.name);
          return fs9.existsSync(path8.join(profileDir, "Cookies")) || platform === "win32" && fs9.existsSync(path8.join(profileDir, "Network", "Cookies"));
        }))
          return true;
      } catch {}
    }
...
function listProfiles(browserName) {
  ...
  for (const platform of getSearchPlatforms()) {
    ...
    if (!dataDir)
      continue;
    const browserDir = path8.join(getBaseDir(platform), dataDir);
    if (!fs9.existsSync(browserDir))
      continue;
    let entries;
    try {
      entries = fs9.readdirSync(browserDir, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const entry of entries) {
      ...
      if (!cookieCandidates.some((p) => fs9.existsSync(p)))
        continue;
      ...
      try {
        ...
        if (fs9.existsSync(prefsPath)) {
          const prefs = JSON.parse(fs9.readFileSync(prefsPath, "utf-8"));
...
function openDbFromCopy(dbPath, browserName) {
  ...
  try {
    fs9.copyFileSync(dbPath, tmpPath);
    const walPath = dbPath + "-wal";
    const shmPath = dbPath + "-shm";
    if (fs9.existsSync(walPath))
      fs9.copyFileSync(walPath, tmpPath + "-wal");
    if (fs9.existsSync(shmPath))
      fs9.copyFileSync(shmPath, tmpPath + "-shm");
    ...
    db.close = () => {
      ...
      try {
        fs9.unlinkSync(tmpPath + "-wal");
      } catch {}
      try {
        fs9.unlinkSync(tmpPath + "-shm");
      } catch {}
    };
    return db;
  } catch {
...
async function getWindowsAesKey(browser) {
  ...
  let localState;
  try {
    localState = JSON.parse(fs9.readFileSync(localStatePath, "utf-8"));
  } catch (err) {
...
function isBrowserRunning(browserName) {
  ...
  return new Promise((resolve6) => {
    ...
    proc.exited.then(async () => {
      ...
      resolve6(out.toLowerCase().includes(exe));
    }).catch(() => resolve6(false));
  });
}
...
var require_filesystem = __commonJS(function(exports, module) {
  ...
  var MAX_LENGTH = 2048;
  var readFileSync5 = (path9) => {
    const fd = fs10.openSync(path9, "r");
    const buffer = Buffer.alloc(MAX_LENGTH);
    const bytesRead = fs10.readSync(fd, buffer, 0, MAX_LENGTH, 0);
    fs10.close(fd, () => {});
    return buffer.subarray(0, bytesRead);
  };
  var readFile = (path9) => new Promise((resolve6, reject) => {
    fs10.open(path9, "r", (err, fd) => {
      if (err) {
        reject(err);
      } else {
        const buffer = Buffer.alloc(MAX_LENGTH);
...
var require_libvips = __commonJS(function(exports, module) {
  /*!
    Copyright 2013 Lovell Fuller and others.
    SPDX-License-Identifier: Apache-2.0
  */
...
var require_sharp = __commonJS(function(exports, module) {
  /*!
    Copyright 2013 Lovell Fuller and others.
    SPDX-License-Identifier: Apache-2.0
  */
...
var require_constructor = __commonJS(function(exports, module) {
  /*!
    Copyright 2013 Lovell Fuller and others.
    SPDX-License-Identifier: Apache-2.0
  */
...
var require_resize = __commonJS(function(exports, module) {
  /*!
    Copyright 2013 Lovell Fuller and others.
    SPDX-License-Identifier: Apache-2.0
  */
...
var require_composite = __commonJS(function(exports, module) {
  /*!
    Copyright 2013 Lovell Fuller and others.
    SPDX-License-Identifier: Apache-2.0
  */
...
var require_operation = __commonJS(function(exports, module) {
  /*!
    Copyright 2013 Lovell Fuller and others.
    SPDX-License-Identifier: Apache-2.0
  */
...
var require_colour2 = __commonJS(function(exports, module) {
  /*!
    Copyright 2013 Lovell Fuller and others.
    SPDX-License-Identifier: Apache-2.0
  */
...
var require_output = __commonJS(function(exports, module) {
  /*!
    Copyright 2013 Lovell Fuller and others.
    SPDX-License-Identifier: Apache-2.0
  */
...
var require_utility = __commonJS(function(exports, module) {
  /*!
    Copyright 2013 Lovell Fuller and others.
    SPDX-License-Identifier: Apache-2.0
  */
...
async function guardScreenshotPath(filePath) {
  const input = readFileSync5(filePath);
  ...
  switch (command) {
    ...
    case "load-html": {
      ...
      for (let i = 0;i < args.length; i++) {
        if (args[i] === "--from-file") {
          ...
          try {
            validateReadPath(path9.resolve(payloadPath));
          } catch {
          ...
          const raw = fs10.readFileSync(payloadPath, "utf8");
          let json;
          try {
            json = JSON.parse(raw);
          } catch (e) {
      ...
      let stat;
      try {
        stat = await fs10.promises.stat(absolutePath);
      } catch (e) {
        if (e.code === "ENOENT") {
      ...
      const buf = await fs10.promises.readFile(absolutePath);
    ...
    case "upload": {
      ...
      for (const fp of filePaths) {
        if (!fs10.existsSync(fp))
          throw new Error(`File not found: ${fp}`);
        if (path9.isAbsolute(fp)) {
          let resolvedFp;
          try {
            resolvedFp = fs10.realpathSync(path9.resolve(fp));
          } catch (err) {
            if (err?.code !== "ENOENT")
              throw err;
            resolvedFp = path9.resolve(fp);
          }
          if (!SAFE_DIRECTORIES.some((dir) => isPathWithin(resolvedFp, dir))) {
            throw new Error(`Path must be within: ${SAFE_DIRECTORIES.join(", ")}`);
          }
        }
        if (path9.normalize(fp).includes("..")) {
      ...
      const fileInfo = filePaths.map((fp) => {
        ...
        return `${path9.basename(fp)} (${stat.size}B)`;
      }).join(", ");
    ...
    case "cookie-import": {
      ...
      try {
        resolvedReal = fs10.realpathSync(resolved);
      } catch {
        try {
          resolvedReal = path9.join(fs10.realpathSync(path9.dirname(resolved)), path9.basename(resolved));
        } catch {}
      }
...
Tip: For scripted imports, use --domain <domain> to scope cookies to a single domain.`;
    ...
    case "download": {
      ...
      if (url.startsWith("blob:")) {
        const dataUrl = await page2.evaluate(async (blobUrl) => {
          try {
            ...
            return new Promise((resolve7, reject) => {
              ...
              reader.onloadend = () => resolve7(reader.result);
        ...
        const tempPath = path9.join(TEMP_DIR, `browse-nav-download-${Date.now()}`);
        await download.saveAs(tempPath);
        buffer = fs10.readFileSync(tempPath);
        ...
        if (outputPath || isBase64) {
          ...
          const finalPath = path9.join(TEMP_DIR, `browse-download-${Date.now()}${ext2}`);
          fs10.renameSync(tempPath, finalPath);
      ...
      const destPath = outputPath || path9.join(TEMP_DIR, `browse-download-${Date.now()}${ext}`);
      validateOutputPath(destPath);
      fs10.writeFileSync(destPath, buffer);
    ...
    case "scrape": {
      ...
      const dir = dirIdx >= 0 ? args[dirIdx + 1] : path9.join(TEMP_DIR, `browse-scrape-${Date.now()}`);
      ...
      for (let i = 0;i < toDownload.length; i++) {
        ...
        try {
          ...
          try {
            fs10.writeFileSync(filePath, body);
          } catch (writeErr) {
      ...
      fs10.writeFileSync(path9.join(dir, "manifest.json"), JSON.stringify(manifest, null, 2));
...
Summary: ${manifest.succeeded} succeeded, ${manifest.failed} failed, ${Math.round(manifest.total_size / 1024)}KB total`;
    ...
    case "archive": {
      const page2 = bm.getPage();
      const outputPath = args[0] || path9.join(TEMP_DIR, `browse-archive-${Date.now()}.mhtml`);
      validateOutputPath(outputPath);
      try {
        ...
        fs10.writeFileSync(outputPath, data);
...
function gstackHome2() {
  return process.env.GSTACK_HOME || path10.join(os8.homedir(), ".gstack");
}
function globalFile() {
  return path10.join(gstackHome2(), "global-domain-skills.jsonl");
}
function projectFile(slug) {
  return path10.join(gstackHome2(), "projects", slug, "learnings.jsonl");
}
...
function getCurrentProjectSlug() {
  ...
  try {
    const slugBin = path11.join(os9.homedir(), ".claude/skills/gstack/bin/gstack-slug");
...
async function readBodyFromArgs(args) {
  ...
  if (fromFileIdx >= 0 && fromFileIdx + 1 < args.length) {
    ...
    const body = await fs12.readFile(filePath, "utf8");
    return body;
  }
...
` + "Action: $B domain-skill save to create one first.");
  ...
  const tmpFile = path12.join(os10.tmpdir(), `gstack-domain-skill-${process.pid}-${Date.now()}.md`);
  await fs12.writeFile(tmpFile, current.body, "utf8");
  const result = spawnSync(editor, [tmpFile], { stdio: "inherit" });
  if (result.status !== 0) {
    await fs12.unlink(tmpFile).catch(() => {});
    throw new Error(`Editor exited with status ${result.status}; no changes saved.`);
  }
  const newBody = await fs12.readFile(tmpFile, "utf8");
  await fs12.unlink(tmpFile).catch(() => {});
...
function defaultTierPaths(opts = {}) {
  ...
  return {
    project: projectRoot ? path13.join(projectRoot, ".gstack", "browser-skills") : null,
    global: path13.join(home, ".gstack", "browser-skills"),
    bundled: path13.join(bundledRoot, "browser-skills")
  };
}
...
function detectBundledRoot() {
  try {
    ...
    if (exec && /\/browse\/dist\/browse$/.test(exec)) {
      return path13.resolve(path13.dirname(exec), "..", "..");
    }
  } catch {}
  ...
  for (const { tier, root } of order) {
    ...
    for (const entry of entries) {
      ...
      if (!fs13.existsSync(skillFile))
        continue;
      try {
        const content = fs13.readFileSync(skillFile, "utf-8");
...
function readBrowserSkill(name, tiers) {
  ...
  for (const { tier, root } of order) {
    if (!root)
      continue;
    const dir = path13.join(root, name);
    const skillFile = path13.join(dir, "SKILL.md");
    if (!fs13.existsSync(skillFile))
      continue;
    try {
...
function tombstoneBrowserSkill(name, tier, tiers) {
  ...
  const src = path13.join(root, name);
  if (!fs13.existsSync(src)) {
    throw new Error(`tombstoneBrowserSkill: skill "${name}" not found in tier "${tier}" at ${src}`);
  }
  const tombstoneDir = path13.join(root, ".tombstones");
  fs13.mkdirSync(tombstoneDir, { recursive: true });
  const ts = new Date().toISOString().replace(/[:.]/g, "-");
  const dst = path13.join(tombstoneDir, `${name}-${ts}`);
  fs13.renameSync(src, dst);
  return dst;
}
var __dirname = "C:\\Users\\ADMIN\\gstack\\browse\\src";
...
function checkDomain(info, url) {
  ...
  try {
    ...
    for (const pattern of info.domains) {
      if (matchDomainGlob(hostname2, pattern))
        return true;
    }
    return false;
  } catch {
...
function matchDomainGlob(hostname2, pattern) {
  if (pattern.startsWith("*.")) {
    const suffix = pattern.slice(1);
    return hostname2.endsWith(suffix) || hostname2 === pattern.slice(2);
  }
...
function handleShow2(args, ctx) {
  ...
  return readFile(path14.join(skill.dir, "SKILL.md"));
}
function readFile(p) {
...
async function handleTest(args, ctx) {
  ...
  if (!skill)
    throw new Error(`Skill "${name}" not found.`);
  const testFile = path14.join(skill.dir, "script.test.ts");
  if (!fs14.existsSync(testFile)) {
...
async function runToFiles(cmd, opts) {
  const dir = fs14.mkdtempSync(path14.join(os12.tmpdir(), "gstack-skill-"));
  const outPath = path14.join(dir, "stdout");
  const errPath = path14.join(dir, "stderr");
  try {
...
function readCappedFile(p, capBytes) {
  const size = fs14.statSync(p).size;
  if (size <= capBytes)
    return { text: fs14.readFileSync(p, "utf-8"), truncated: false };
  const fd = fs14.openSync(p, "r");
  try {
    const buf = Buffer.alloc(capBytes);
    const read = fs14.readSync(fd, buf, 0, capBytes, 0);
...
async function spawnSkill(opts) {
  ...
  try {
    ...
    const scriptPath = path14.join(opts.skill.dir, "script.ts");
...
function resolveMinimalPath() {
  ...
  if (bunPath && bunPath.includes("/bun")) {
    const dir = path14.dirname(bunPath);
    return `${dir}:${fallback}`;
  }
  return fallback;
}
...
function quarantineCorrupt(filePath) {
  try {
    fs15.renameSync(filePath, `${filePath}.corrupt`);
  } catch {
...
function parsePdfFromFile(payloadPath) {
  try {
    validateReadPath(path15.resolve(payloadPath));
  } catch {
  ...
  const raw = fs16.readFileSync(payloadPath, "utf8");
  let json;
  try {
    json = JSON.parse(raw);
  } catch (err) {
    ...
    case "focus": {
      ...
      try {
        ...
        for (const appName of appNames) {
          try {
            execSync2(`osascript -e 'tell application "${appName}" to activate'`, { stdio: "pipe", timeout: 3000, windowsHide: true });
            activated = true;
            break;
          } catch (err) {
...
`);
    ...
    case "inbox": {
      ...
      let gitRoot;
      try {
        gitRoot = execSync2("git rev-parse --show-toplevel", { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"], windowsHide: true }).trim();
      } catch (err) {
        if (err?.status === undefined && !err?.message?.includes("Command failed"))
          throw err;
        return "Not in a git repository — cannot locate inbox.";
      }
      const inboxDir = path15.join(gitRoot, ".context", "sidebar-inbox");
      if (!fs16.existsSync(inboxDir))
        return "Inbox empty.";
      const files = fs16.readdirSync(inboxDir).filter((f) => f.endsWith(".json") && !f.startsWith(".")).sort().reverse();
      if (files.length === 0)
        return "Inbox empty.";
      const messages = [];
      for (const file of files) {
        try {
          const data = JSON.parse(fs16.readFileSync(path15.join(inboxDir, file), "utf-8"));
          messages.push({
      ...
      if (args.includes("--clear")) {
        for (const file of files) {
          try {
            fs16.unlinkSync(path15.join(inboxDir, file));
          } catch (err) {
            if (err?.code !== "ENOENT")
              throw err;
          }
...
`);
    ...
    case "state": {
      ...
      const stateDir = path15.join(config.stateDir, "browse-states");
      mkdirSecure(stateDir);
      const statePath = path15.join(stateDir, `${name}.json`);
      if (action === "save") {
        const state = await bm.saveState();
        const saveData = {
          version: 1,
...
⚠️  Cookies stored in plaintext. Delete when no longer needed.`;
      ...
      if (action === "load") {
        ...
        const data = JSON.parse(fs16.readFileSync(statePath, "utf-8"));
        if (!Array.isArray(data.cookies) || !Array.isArray(data.pages)) {
...
function checkPortAvailable(port, hostname2 = "127.0.0.1") {
...
function resolveEgressHome(env = process.env) {
  ...
  if (configured)
    return path17.resolve(configured);
  return path17.join(env.HOME || os13.homedir(), ".gstack");
}
function egressLedgerPath(home) {
  return path17.join(home, "security", "egress.jsonl");
}
...
function lastRawLine(ledger) {
  ...
  try {
    ...
    fs18.readSync(fd, buffer, 0, length, size - length);
...
function warnLedgerSizeOnce(ledger) {
  ...
  let size;
  try {
    size = fs18.statSync(ledger).size;
  } catch {
    return;
  }
...
function appendChained(homeOrNull, record, env) {
  ...
  try {
    fs18.mkdirSync(path17.dirname(ledger), { recursive: true, mode: 448 });
    return withLedgerLock(ledger, () => {
      ...
      const existed = fs18.existsSync(ledger);
      fs18.appendFileSync(ledger, `${line}
`, { mode: 384 });
      if (!existed)
        fs18.chmodSync(ledger, 384);
...
function isDisplayFree(displayNum) {
  try {
    ...
    return !fs19.existsSync(`/tmp/.X11-unix/X${displayNum}`) && !fs19.existsSync(`/tmp/.X${displayNum}-lock`);
  }
}
...
function readPidCmdline(pid) {
  try {
    return fs19.readFileSync(`/proc/${pid}/cmdline`, "utf-8").replace(/\0/g, " ").trim();
  } catch {
    return "";
  }
}
function readPidArgv0(pid) {
  try {
    const raw = fs19.readFileSync(`/proc/${pid}/cmdline`, "utf-8");
...
var init_tunnel_denial_log = __esm(() => {
  init_file_permissions();
  LOG_DIR = path18.join(os14.homedir(), ".gstack", "security");
  LOG_PATH = path18.join(LOG_DIR, "attempts.jsonl");
  writeTimestamps = [];
});
...
function resolveNgrokAuthtoken() {
  ...
  const ngrokEnvPath = path19.join(home, ".gstack", "ngrok.env");
  ...
  const ngrokConfigs = [
    path19.join(home, "Library", "Application Support", "ngrok", "ngrok.yml"),
    path19.join(home, ".config", "ngrok", "ngrok.yml"),
    path19.join(home, ".ngrok2", "ngrok.yml")
  ];
...
function readTerminalPort() {
  try {
    const f = path19.join(path19.dirname(config.stateFile), "terminal-port");
...
function readTerminalInternalToken() {
  try {
    const f = path19.join(path19.dirname(config.stateFile), "terminal-internal-token");
...
function buildFetchHandler(cfg) {
  ...
  if (ownsTerminalAgent) {
    agentWatchdogInterval = setInterval(() => {
      ...
      if (agentRespawnGuardTripped)
        return;
      const stateDir = path19.dirname(cfg.config.stateFile);
  ...
  async function shutdown(exitCode = 0) {
    ...
    if (ownsTerminalAgent) {
      try {
        const stateDir = path19.dirname(config.stateFile);
        const record = readAgentRecord(stateDir);
        if (record)
      ...
      safeUnlinkQuiet(path19.join(path19.dirname(config.stateFile), "terminal-port"));
      safeUnlinkQuiet(path19.join(path19.dirname(config.stateFile), "terminal-internal-token"));
      safeUnlinkQuiet(agentRecordPath(path19.dirname(config.stateFile)));
    }
    try {
      detachSession();
    } catch (err) {
    ...
    if (isSessionPersistEnabled()) {
      const finalSnapshot = persistSessionState(cfgBrowserManager, path19.join(config.stateDir, SESSION_STATE_FILE)).catch((err) => {
        console.warn(`[browse] SESSION_PERSIST_FAILED at shutdown: ${err?.message ?? err}`);
      });
      await Promise.race([
        finalSnapshot,
        new Promise((resolve11) => setTimeout(resolve11, 2000))
      ]);
    }
  ...
  const makeFetchHandler = (surface) => async (req) => {
    ...
    if (url.pathname === "/extension-token" && req.method === "POST") {
      let hostname2 = null;
      try {
        hostname2 = new URL(`http://${req.headers.get("host") ?? ""}`).hostname;
      } catch (err) {
        if (!(err instanceof TypeError))
          throw err;
      }
      const originOk = req.headers.get("origin") === `chrome-extension://${GSTACK_EXTENSION_ID}`;
      const hostOk = hostname2 === "127.0.0.1" || hostname2 === "localhost";
      if (!originOk || !hostOk) {
    ...
    if (url.pathname.startsWith("/token/") && req.method === "DELETE") {
      ...
      const tabsReleased = browserManager2.releaseClientTabs(clientId).length;
    ...
    if (url.pathname === "/pair" && req.method === "POST") {
      ...
      try {
        ...
        if (priorSession && grantReducesAccess(priorSession, grant)) {
          ...
          const tabsReleased = browserManager2.releaseClientTabs(pairBody.clientId).length;
          ...
          if (!priorSession)
            browserManager2.releaseClientTabs(pairBody.clientId);
        }
    ...
    if (url.pathname === "/file" && req.method === "GET") {
      ...
      return new Response(Bun.file(filePath), {
        headers: {
          ...
          "Content-Disposition": `inline; filename="${path19.basename(filePath)}"`,
          "Cache-Control": "no-cache"
        }
      });
    }
...
async function start() {
  ...
  const state2 = {
    ...
    serverPath: path19.resolve(__browseNodeSrcDir, "server.ts"),
  ...
  fs20.writeFileSync(tmpFile, JSON.stringify(state2, null, 2), { mode: 384 });
  ...
  if (!skipBrowser && isSessionPersistEnabled() && browserManager.getConnectionMode() === "launched") {
    const sessionStatePath = path19.join(config.stateDir, SESSION_STATE_FILE);
  ...
  try {
    const stateDir = path19.join(config.stateDir, "browse-states");
    if (fs20.existsSync(stateDir)) {
      const SEVEN_DAYS = 604800000;
      for (const file of fs20.readdirSync(stateDir)) {
        const filePath = path19.join(stateDir, file);
        const stat = fs20.statSync(filePath);
...
var init_server = __esm(() => {
  ...
  if (BROWSE_PARENT_PID > 0 && !IS_HEADED_WATCHDOG) {
    setInterval(parentWatchdogTick, 15000);
  ...
  if (__require.main == __require.module) {
    start().catch((err) => {
      ...
      try {
        const errorLogPath = path19.join(config.stateDir, "browse-startup-error.log");
        mkdirSecure(config.stateDir);
